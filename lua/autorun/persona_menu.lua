...
if CLIENT then
	...
	hook.Add("PopulateToolMenu","Persona_MainMenu",function()
		...
		spawnmenu.AddToolMenuOption("Persona","Admin Settings","Cheats","Cheats","","",function(Panel)
			...
			Panel:AddControl("TextBox",{Label = "Persona Points",Command = "persona_i_ply_givepp",WaitForEnter = "0"})
			Panel:AddControl("Button",{Label = "Give Persona Points",Command = "persona_ply_givepp"})
		end,{})
	end)
	...
	CreateClientConVar("persona_i_ply_givepp","0",false,true)
...
local function persona_ply_givepp(ply)
	if ply:IsAdmin() or ply:IsSuperAdmin() then
		PXP.GivePoints(ply,GetConVarNumber("persona_i_ply_givepp"))
	end
end
concommand.Add("persona_ply_givepp",persona_ply_givepp)
